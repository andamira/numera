    ...
    ($rhs:ident) => {
        impl<N: Number + Signed + Mul<Output = N>> Mul<$rhs<N>> for Integer<N> {
            ...
            fn mul(self, other: $rhs<N>) -> Self::Output {
                Integer::new(self.0.mul(other.0))
            }
        }
    };
}
...
macro_rules! impl_mul_smaller_prim {
    (all: $t:ident, $doc:literal, $( ($n1:ty, $n2:ty ) ),+) => {
        $( impl_mul_smaller_prim![$t, $doc, ( $n1, $n2 )]; )+
    };
    ($t:ident, $doc:literal, ($n1:ty, $n2:ty) ) => {
        paste::paste! {
            #[doc = "`Z<" $n1 "> × " $n2 " = Z<" $n1 ">`" $doc ]
            impl Mul<$n2> for $t<$n1> {
                type Output = $t<$n1>;
...
#[rustfmt::skip]
impl_mul_smaller_prim![all: Integer, "",
    (i16, i8),
    (i32, i16), (i32, i8),
    (i64, i32), (i64, i16), (i64, i8),
    (i128, i64), (i128, i32), (i128, i16), (i128, i8) ];
...
mod mul_ibig {
    ...
    impl Mul<IBig> for Integer<IBig> {
        ...
        fn mul(self, other: IBig) -> Integer<IBig> {
            Integer::new(self.0.mul(other))
        }
    }
